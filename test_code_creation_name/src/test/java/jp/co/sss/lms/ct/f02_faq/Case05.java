...
public class Case05 {
	...
	void test01() throws Exception {
		...
		Files.copy(file.toPath(), Paths.get("./evidence/Case05/No.1.png"));
	...
	void test02() throws Exception {
		...
		Files.copy(file2.toPath(), Paths.get("./evidence/Case05/No.2.png"));
	...
	void test03() throws Exception {
		...
		Files.copy(file3.toPath(), Paths.get("./evidence/Case05/No.3.png"));
	...
	void test04() throws Exception {
		...
		Files.copy(file4.toPath(), Paths.get("./evidence/Case05/No.4.png"));
	...
	void test05() throws Exception {
		// 1. 検索用キーワードを設定（画面に実在する質問の単語、例として「キャンセル」を設定）
		String searchWord = "研修";
		// 2. キーワード検索の入力欄を取得して入力
		final WebElement keywordInput = webDriver.findElement(By.name("keyword"));
		keywordInput.sendKeys(searchWord);

		assertEquals(searchWord, keywordInput.getAttribute("value"), "検索キーワードが正しく入力されていません。");
		...
		String pageText = webDriver.findElement(By.tagName("body")).getText();
		assertTrue(pageText.contains(searchWord), "検索したキーワードを含む結果が表示されていません。");

		// エビデンス保存（No.5: 検索結果画面）
		File file5 = ((TakesScreenshot) webDriver).getScreenshotAs(OutputType.FILE);
		Files.copy(file5.toPath(), Paths.get("./evidence/Case05/No.5.png"));
		System.out.println("No.5: キーワード検索結果の確認とエビデンス保存を完了しました。");
	...
	void test06() throws Exception {
		// 1. クリアボタンを取得してクリック
		final WebElement clearButton = webDriver.findElement(By.cssSelector("input[value='クリア']"));
		clearButton.click();
		// クリア処理を待つ
		Thread.sleep(2000);

		// 2. 【検証】入力欄が空（""）になっていることを確認
		final WebElement keywordInput = webDriver.findElement(By.name("keyword"));
		assertEquals("", keywordInput.getAttribute("value"), "クリアボタン押下後もキーワードが消去されていません。");

		// エビデンス保存（No.6: クリア後の初期画面）
		File file6 = ((TakesScreenshot) webDriver).getScreenshotAs(OutputType.FILE);
		Files.copy(file6.toPath(), Paths.get("./evidence/Case05/No.6.png"));
		System.out.println("No.6: キーワードクリア確認とエビデンス保存を完了しました。");
	}
